...
def biologic_importer(read_filepath):
    ...
    if not set(mandatory_state_columns).issubset(set(raw.keys())):
        raise Exception('Not all mandatory state columns present in the data. Please ensure Ns, '
    ...
    unwanted_state_columns = ['mode', 'ox/red', 'cantrol changes', 'Ns changes', 'counter inc.',
                              'I Range', 'control/V/mA']
    ...
    stacked = raw.melt('RecNo', value_name='value').sort_values('RecNo').reset_index(drop=True)
    ...
    state_repeated = pd.concat([state_data] * number_of_vars).sort_index().reset_index(drop=True)\
        .drop('RecNo', axis=1)
